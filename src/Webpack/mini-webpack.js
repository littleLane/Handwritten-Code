...
function createAsset(filename) {
  ...
  //将字符串为ast（抽象语法树， 这个是编译原理的知识，说得简单一点就是，可以把js文件里的代码抽象成一个对象，代码的信息会存在对象中）
  //babylon 这个工具是是负责解析字符串并生产ast。
  const ast = babylon.parse(content, {
    sourceType: "module",
  });
  fs.writeFileSync(filename + ".json", JSON.stringify(ast));
...
function createGraph(entry) {
  ...
  fs.writeFileSync("./queue.js", JSON.stringify(queue));
  return queue;
}
